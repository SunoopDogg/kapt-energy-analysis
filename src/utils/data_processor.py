...
def update_metropolitan_data(file_name, source_folder='xlsx', target_folder='csv',
    ...
    Args:
        ...
        metropolitan_regions (list): 수도권으로 간주할 지역 목록. 기본값은 ['서울특별시']

    Returns:
        str: 생성된 CSV 파일 경로
    """
    if metropolitan_regions is None:
        metropolitan_regions = ['서울특별시']
    ...
    # 파일명 생성 (확장자 변경)
    output_file_name = os.path.splitext(file_name)[0] + output_suffix + '.csv'
...
if __name__ == "__main__":
    ...
    try:
        # 수도권 데이터 추출 및 저장
        updated_file = update_metropolitan_data(file_name)
